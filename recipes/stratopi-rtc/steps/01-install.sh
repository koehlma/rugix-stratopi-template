#!/bin/bash

set -euo pipefail

SRC_DIR="/tmp/rtc-pcf2131"

BUILD_DEPS="build-essential device-tree-compiler"

# Record which build dependencies are already installed so we don't remove them later.
ALREADY_INSTALLED=()
for pkg in $BUILD_DEPS; do
    if dpkg-query -W -f='${Status}' "$pkg" 2>/dev/null | grep -q "install ok installed"; then
        ALREADY_INSTALLED+=("$pkg")
    fi
done

apt-get install -y $BUILD_DEPS

# Build the kernel module for each installed kernel.
for KVER_DIR in /lib/modules/*/; do
    KVER=$(basename "$KVER_DIR")
    KDIR="/lib/modules/$KVER/build"
    [ -d "$KDIR" ] || continue
    echo "Building rtc-pcf2131.ko for kernel ${KVER}..."
    make -C "$KDIR" M="$SRC_DIR" modules
    install -m 644 "$SRC_DIR/rtc-pcf2131.ko" "/lib/modules/$KVER/"
    depmod -a "$KVER"
    make -C "$KDIR" M="$SRC_DIR" clean
done

BOOT_DIR="${RUGIX_LAYER_DIR}/roots/boot"

# Compile and install the device tree overlay.
mkdir -p "$BOOT_DIR/overlays"
dtc -@ -Hepapr -I dts -O dtb \
    -o "$BOOT_DIR/overlays/rtc-pcf2131.dtbo" \
    "$SRC_DIR/rtc-pcf2131.dts"

# Enable the device tree overlay on the boot partition.
CONFIG="$BOOT_DIR/config.txt"
if [ ! -f "$CONFIG" ]; then
    echo "ERROR: config.txt not found at $CONFIG" >&2
    exit 1
fi
if ! grep -q "^dtoverlay=rtc-pcf2131$" "$CONFIG"; then
    echo "dtoverlay=rtc-pcf2131" >> "$CONFIG"
fi
# Disable the Pi 5's built-in RTC to avoid conflicts. Harmless on Pi 4/CM4.
if ! grep -q "^dtparam=rtc=off$" "$CONFIG"; then
    echo "dtparam=rtc=off" >> "$CONFIG"
fi

# Auto-load the module at boot.
echo "rtc-pcf2131" > /etc/modules-load.d/rtc-pcf2131.conf

# Clean up: only remove build dependencies that were not already installed.
rm -rf "$SRC_DIR"
TO_REMOVE=()
for pkg in $BUILD_DEPS; do
    skip=false
    for kept in "${ALREADY_INSTALLED[@]}"; do
        if [ "$pkg" = "$kept" ]; then
            skip=true
            break
        fi
    done
    if [ "$skip" = false ]; then
        TO_REMOVE+=("$pkg")
    fi
done
if [ ${#TO_REMOVE[@]} -gt 0 ]; then
    apt-get purge -y "${TO_REMOVE[@]}"
    apt-get autoremove -y --purge
fi
